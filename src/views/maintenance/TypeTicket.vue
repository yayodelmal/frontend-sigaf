<template>
  <div>
    <base-card color="blueS" class="px-5 py-3" title="Tipo de ticket">
      <v-data-table
        :headers="headers"
        :items="items"
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    editedItem: new TypeTicket(),
    defaultItem: new TypeTicket(),
    message: '',
  ...
  computed: {
    ...mapGetters({ items: 'typeTicket/typeTickets' }),
    descriptionErrors() {
      const errors = []
  ...
  methods: {
    ...mapActions({
      fetchItems: 'typeTicket/fetchTypeTickets',
      postItem: 'typeTicket/postTypeTicket',
      putItem: 'typeTicket/putTypeTicket',
