<template>
  <div>
    <base-card color="blueS" class="px-5 py-3" title="Motivo de ticket">
      <v-data-table
        :headers="headers"
        :items="items"
...
<script>
import MotiveTicket from '../../models/MotiveTicket'
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    editedItem: new MotiveTicket(),
    defaultItem: new MotiveTicket(),
    message: '',
  ...
  computed: {
    ...mapGetters({ items: 'motiveTicket/motiveTickets' }),
    descriptionErrors() {
      const errors = []
