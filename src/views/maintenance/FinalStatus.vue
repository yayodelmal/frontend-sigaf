<template>
  <div>
    <base-card color="blueS" class="px-5 py-3" title="Estado final de alumno">
      <v-data-table
        :headers="headers"
        :items="items"
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    editedItem: new FinalStatus(),
    defaultItem: new FinalStatus(),
    message: '',
  ...
  computed: {
    ...mapGetters({ items: 'finalStatus/finalStatuses' }),
    descriptionErrors() {
      const errors = []
  ...
  methods: {
    ...mapActions({
      fetchItems: 'finalStatus/fetchFinalStatuses',
      postItem: 'finalStatus/postFinalStatus',
      putItem: 'finalStatus/putFinalStatus',
      removeItem: 'finalStatus/deleteFinalStatus'
    }),
    editItem(item) {
