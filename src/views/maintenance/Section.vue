<template>
  <div>
    <base-card color="blueS" class="px-5 py-3" title="Secciones">
      <v-data-table
        :headers="headers"
        :items="items"
...
export default {
  ...
  data: () => ({
    ...
    editedIndex: -1,
    editedItem: new Section(),
    defaultItem: new Section(),
    message: '',
  ...
  computed: {
    ...mapGetters({ items: 'section/sections' }),
    descriptionErrors() {
      const errors = []
  ...
  methods: {
    ...mapActions({
      fetchItems: 'section/fetchSections',
      postItem: 'section/postSection',
      putItem: 'section/putSection',
      removeItem: 'section/deleteSection'
    }),
    editItem(item) {
