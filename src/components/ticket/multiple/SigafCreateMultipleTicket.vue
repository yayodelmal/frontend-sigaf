  ...
  >
    <v-card>
      <v-card-title>
        ...
        <v-spacer />
        <v-btn color="blueS" text>Cancelar</v-btn>
      </v-card-title>
      <v-card-text>
                          ...
                          >
                            <v-card>
                              <v-card-text>
                                ...
                                ></span>
                                {{ timeline.text }}
                                <div v-if="timeline.hasItems" class="caption">
                                  <ol>
                                    <li
...
export default {
  ...
  data: () => ({
    ...
    text: `<div><h3><strong>Estimado Roberto,</strong></h3>
      <p align="justify">Junto con saludar, informo que no podemos procesar su solicitud de incorporación a la academia de inteligencia naval de Lota. Estamos full vacantes y ya no tenemos espacio.</p>
      <p>Atento a sus comentarios,</p>
      <p>Saludos cordiales,</p>
      <p>&nbsp;</p>
      <p>Departamento de Inteligencia Naval</p>
      <p>DIRE.</p></div>`,
  ...
  methods: {
    ...
    filterUsersByCategoriesTicket() {
      ...
      if (this.selectedClassrooms.length === 0) {
        this.courseRegisteredUsers.forEach(userCourse => {
          this.userRegisteredFiltered.push(userCourse)
        })
      } else {
        this.courseRegisteredUsers.forEach(userCourse => {
          this.selectedClassrooms.forEach(classroom => {
        ...
        if (success) {
          if (this.editedDetailTicketItem.statusDetail.id !== 0) {
            ...
            if (response.success) {
              ...
              const res = await this.postMailMultipleTicket(payload)
